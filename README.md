...
The graph below plots the same data as the ground truth clustering above, but this time the yellow points are predicted fraud, not actual fraud. The unimpressive overlap of the two graphs illustrates the point made by the pairwise measures. 
...
The graph below plots the same data as the ground truth clustering at the top of the section, but this time the yellow points are predicted fraud, not actual fraud. Even though the graph only shows the top three principal components, the poor correspondence of this graph to the ground truth one shows that this clustering lacks much predictive power.
